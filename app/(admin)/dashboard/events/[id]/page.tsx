...
export default function EventDetailsPage() {
    ...
    const handleUpdateEvent = async () => {
        ...
        try {
            ...
            showToast('Error updating event: ' + err.message, 'error')
        } finally {
            setUpdating(false)
        }
    }
    ...
    return (
        <div>
            ...
            <div className="card" style={{ marginBottom: '2rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', flexWrap: 'wrap', gap: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                        {isEditing ? (
                            <>
                                <button className="btn btn-outline" onClick={() => setIsEditing(false)} disabled={updating}>Cancel</button>
                                <button className="btn btn-primary" onClick={handleUpdateEvent} disabled={updating}>
            ...
            {/* ONLY SHOW Stats and Redemption Logs when NOT Editing (or show below?) 
                Review request: "only by editing the event there should be an option to add or generate"
                Interpretation: Generate Coupons should only be visible if isEditing is true.
            */}

            {isEditing && (
                <div style={{ border: '1px dashed var(--card-border)', padding: '1.5rem', borderRadius: '0.5rem', marginBottom: '2rem' }}>
                    <h3 className="label" style={{ marginBottom: '1rem', color: 'var(--primary)' }}>Edit Actions</h3>
